...
type ReportDoc = {
  ...
  periodKey?: string
  reportDate: FirebaseFirestore.Timestamp
  totalAmount?: number
...
export const processSalesReport = functions
  .region('europe-west1')
  .firestore.document(
    'tenants/{tenantId}/workspaces/{workspaceId}/salesReports/{reportId}',
  )
  .onWrite(async (change, context) => {
    const db = admin.firestore()
    const bucket = admin.storage().bucket()
    
    const after = change.after.data() as ReportDoc | undefined
    if (!after) return
    ...
    if (after.status !== 'uploaded' && after.status !== 'processing') return
    ...
    try {
      ...
      const menuGroupLabel = new Map<string, string>()
      menuGroupConfig.data()?.groups?.forEach((group: { id: string; label: string }) => {
      ...
      const matchedLines = parsed.map((row) => {
        ...
        let product: (FirebaseFirestore.DocumentData & { id: string }) | undefined = undefined
        ...
        if (!product) {
          for (const [key, value] of productMap.entries()) {
            ...
            const productData = value as FirebaseFirestore.DocumentData & { id: string }
    ...
    if (unmapped.size > 0) {
      ...
      const periodKey =
        after.periodKey ?? after.reportDate.toDate().toISOString().slice(0, 7)
      ...
      const categoryTotals = new Map<
        string,
        { qty: number; amount: number; menuGroupId: string; menuGroupLabel: string }
      >()
      ...
      productTotals.forEach((aggregate, key) => {
        ...
        const metricRef = metricsCollection.doc(
          `monthlyProductSummary_${periodKey}_${productId}`,
        )
      ...
      categoryTotals.forEach((aggregate, key) => {
        ...
        const metricRef = metricsCollection.doc(
          `monthlyCategorySummary_${periodKey}_${menuGroupId}`,
        )
        metricsBatch.set(
          metricRef,
          {
...
async function parseReportFile(filePath: string): Promise<ParsedRow[]> {
  if (filePath.endsWith('.csv')) {
    const content = fs.readFileSync(filePath, 'utf8')
    const parsed = Papa.parse<ParsedRow>(content, { header: true })
    ...
    row.eachCell((cell, colNumber) => {
      ...
      if (cellText && cellText !== '') {
        ...
        if ('result' in value) {
          ...
          rowData[header] = String((value as any).text)
        } else {
          rowData[header] = String(value)
        }
      } else {
